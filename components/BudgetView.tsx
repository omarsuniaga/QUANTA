...
import { ArrowLeft, Calendar, Plus, RefreshCw, PiggyBank, Calculator as CalcIcon } from 'lucide-react';
...
export const BudgetView: React.FC<BudgetViewProps> = ({
  ...
  const handleSubmit = useCallback((e: React.FormEvent) => {
    ...
    onSave(budgetData);
    onBack();
  }, [formData, budget, expenseCategories, language, onSave, onBack, validateForm]);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white dark:bg-slate-900 flex flex-col">
      ...
      <div className="sticky top-0 z-10 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 p-4 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 border-purple-100 dark:border-purple-800">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 p-4 flex gap-3">
        <button
          type="button"
          onClick={onBack}
          className="flex-1 py-3 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-bold rounded-2xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
        >
          {language === 'es' ? 'Cancelar' : 'Cancel'}
        </button>
        <button
          ...
          className="flex-1 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-bold rounded-2xl shadow-lg transition-all"
        >
          {budget
